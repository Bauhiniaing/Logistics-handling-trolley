...
void LCD_SHOWER(void)
{  
	
	//使用宇树电机时，不要显示字符串!!!!!!!!!!! 
        //   LCD_Num(0,0,delay_flag,3,16);//光电1guangdian_flag
        //   LCD_Num(40,0,move_stop_flag,3,16);//光电2guangdian_flag2
        //   LCD_Num(90,0,tim4_delay,5,16);
        //   LCD_Num(140,0,is_delay_triggered,5,16);
        //   LCD_Num(170,40,flafjh,1,24);
    //    LCD_Num(10,20,flag_2,3,24);//陀螺仪stcAngle.Angle[2] (Go1Motor[4].Pos - GOset[4].GO_setpos)
    //    LCD_Num(20,40,(laser_distance )/10,4,16);//y激光测距
    //    LCD_Num(20,80,(laser_distance1 )/10,4,16);//x激光测距
	  // LCD_Num(60,0,stcAngle.Angle[2],5,24);//灯距离	
      // LCD_Num(70,30, stcAngle.Angle[2],5,24);//灯左右坐标
 //      LCD_Num(80,60,zuobiao_pan_date_x,5,24);//球左右坐标
    //   LCD_Num(90,90,zuobiao_pan_date_y,5,24);//球左右坐标
    
    //   LCD_Num(120,180,laser_distance ,4,24);//go初始化标志
//     LCD_Num(120,220,zuobiao_date_z ,5,16);//盘左下
//    LCD_Num(130,240,zuobiao_date_x,5,16);//盘右下
    //  LCD_Num(140,260,Go1Motor[1].Pos,3,24);//盘右上
    //  LCD_Num(150,280,Go1Motor[2].Pos,3,24);//盘左上
    //   LCD_Num(160,220,Go1Motor[3].Pos,3,24);//锤子
    //  LCD_Num(170,320,Go1Motor[4].Pos,3,24);//俯仰
